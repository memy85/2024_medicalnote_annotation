...
def load_results(cv_idx, real_name, shot, prompt_name = None) :
...
def main() :
    ...
    if do_finetune : 
        ...
        # prompt_name = "generic"
        if mimic_flag : 
            real_name = f"{model_name}_mimic_finetuned"
        else :
            pass
    else : 
    ...
    for rank in [3,5,10] :
        ...
        for cv_idx, (train_idx, test_idx) in enumerate(cv10) :
            ...
            #%% 
            gold_dataset = top10_dataset[top10_dataset.noteid.isin(test_file_ids)].sort_values(by="ranking").copy()
        ...
        evaluation_df.to_pickle(DATA_PATH.joinpath(f"{topn}_{real_name}_{shot}_df.pkl"))
        
        if prompt_name :
            with open(DATA_PATH.joinpath(f"{topn}_{real_name}_{shot}_{prompt_name}_evaluation.pkl"), 'wb') as f :
                pickle.dump(results, f)
        else :
            with open(DATA_PATH.joinpath(f"{topn}_{real_name}_{shot}_evaluation.pkl"), 'wb') as f :
                pickle.dump(results, f)
