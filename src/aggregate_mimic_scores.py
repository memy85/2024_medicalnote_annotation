...
def load_results(cv_idx, real_name, shot, prompt_name) :

    with open(DATA_PATH.joinpath(f"{real_name}_{cv_idx}_{shot}_prediction.pkl"), "rb") as f :
...
def parse_arguments() :
    ...
    parser.add_argument("--aug_size", type=int, help='augment size', default=10)
...
def main() :
    ...
    shot = "zeroshot"
    prompt_name = "generic"
    real_name = f"{model_name}_mimic_{aug_size}_finetuned"
    ...
    for rank in [3,5,10] :
        ...
        evaluation_df = em.calculate_the_scores_with_confidence_intervals(results) 

        evaluation_df.to_pickle(DATA_PATH.joinpath(f"{topn}_{real_name}_zeroshot_df.pkl"))

        with open(DATA_PATH.joinpath(f"{topn}_{real_name}_zeroshot_evaluation.pkl"), 'wb') as f :
